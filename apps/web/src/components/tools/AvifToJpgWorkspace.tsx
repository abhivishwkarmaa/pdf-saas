...
interface AvifToJpgWorkspaceProps {
  tool: ToolDefinition;
}
...
export function AvifToJpgWorkspace({ tool }: AvifToJpgWorkspaceProps) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    toast.success(`Added ${newItems.length} AVIF image(s)!`);
  };
  ...
  // Convert single AVIF to JPG using client canvas with server fallback
  const convertSingleAvifToJpg = async (
    item: ConvertItem,
    ...
    try {
      return await new Promise<Blob>((resolve, reject) => {
        ...
        img.onerror = () => reject(new Error("Browser cannot decode AVIF directly"));
        img.src = item.previewUrl;
      });
    } catch {
      // Fallback to server API POST `/api/process/avif-to-jpg`
      const formData = new FormData();
      formData.append("files", item.file);
      formData.append("options", JSON.stringify({ quality: qualityPercent }));

      const res = await fetch("/api/process/avif-to-jpg", {
        method: "POST",
        body: formData,
      });

      if (!res.ok) throw new Error("AVIF conversion failed");
      return await res.blob();
    }
  };

  // Batch convert all AVIF to JPG
  const handleConvertAll = async () => {
    if (images.length === 0) {
      toast.error("Please upload at least one AVIF image.");
      return;
    }

    setProcessing(true);
    setStatusMessage("Converting modern AVIF images to standard JPG... 0%");
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        const convertedBlob = await convertSingleAvifToJpg(item, quality);
      ...
      setImages(updated);
      setIsConverted(true);
      toast.success("All AVIF images converted to JPG successfully!");
    } catch (err) {
      console.error(err);
      toast.error("Failed to convert one or more AVIF images.");
    } finally {
      setProcessing(false);
    }
  };
  ...
  const downloadAllAsZip = async () => {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `converted_avif_jpgs_${Date.now()}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      ...
      {images.length === 0 && (
        <div className="mb-8 text-center">
          ...
          >
            ...
            Modern AVIF to Universal JPG Converter
          </span>
        ...
        <label className="upload-dropzone upload-dropzone-image w-full relative group cursor-pointer">
          ...
          <span className="text-center">
            <p className="text-base font-semibold text-zinc-800 dark:text-zinc-100">
              Click or drag AVIF images here to convert to JPG
            </p>
            <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
              Make AV1-encoded AVIF images compatible with all devices and editors.
            </p>
            <p className="mt-2 text-[11px] font-medium text-zinc-400">
              Max {tool.maxFiles} images · Up to {tool.maxMb} MB each · Fast & Private
            </p>
          </span>
        </label>
      )}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-zinc-900 dark:text-white">
                    {images.length} AVIF Image{images.length === 1 ? "" : "s"} Loaded
                  </h3>
            ...
            <div className="flex flex-col gap-5 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
              <div>
                ...
                <p className="text-xs text-zinc-500">
                  Configure output JPG compression and color preservation.
                </p>
              </div>
              ...
              <div className="rounded-xl border border-zinc-200 bg-zinc-50/80 p-3.5 dark:border-zinc-800 dark:bg-zinc-900/50 space-y-2 text-xs text-zinc-600 dark:text-zinc-400">
                <div className="flex items-center gap-2">
                  ...
                  <span>Converts AV1-compressed image stream to standard JPEG</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="h-4 w-4 text-emerald-600 dark:text-emerald-400 shrink-0" />
                  <span>100% compatible with photo galleries and social apps</span>
                </div>
              </div>
              ...
              >
                {processing ? (
                  <>
                    ...
                    <span>Converting AVIF to JPG...</span>
                  </>
                ) : (
                  <>
                    <FileType className="h-4 w-4" />
                    <span>Convert {images.length} AVIF{images.length === 1 ? "" : "s"} to JPG</span>
                  </>
                )}
              </button>
            </div>
