...
type RepairStrategy = "auto" | "deep" | "linearize";
...
const REPAIR_PRESETS: StrategyPreset[] = [
  {
    id: "auto",
    title: "Smart Auto-Repair (Recommended)",
    description: "Automatically analyzes document corruption, reconstructs cross-reference (XREF) tables, fixes invalid trailers, and recovers unreadable pages.",
    badge: "Recommended",
    isPopular: true,
  },
  {
    id: "deep",
    title: "Deep Structure Reconstruction",
    description: "Performs full byte-stream salvage, purges damaged metadata, rebuilds font descriptors, and fixes truncated streams.",
    badge: "Deep Recovery",
  },
  {
    id: "linearize",
    title: "Linearize & Stream Recovery",
    description: "Re-indexes all internal PDF pointers, repairs stream defragmentation, and optimizes structure for fast web viewing.",
    badge: "Fast View Fix",
  },
];

export function RepairPdfWorkspace({ tool }: RepairPdfWorkspaceProps) {
  ...
  const [selectedStrategy, setSelectedStrategy] = useState<RepairStrategy>("auto");
  ...
  // Load PDF and attempt preview rendering
  useEffect(() => {
    ...
    const renderPdfPages = async () => {
      setLoadingPdf(true);
      setLoadingProgress("Diagnosing PDF structure...");
      setPages([]);
      ...
      try {
        ...
        for (let i = 1; i <= total; i++) {
          ...
          setLoadingProgress(`Reading page ${i} of ${total}...`);
          ...
          try {
            ...
            // Damaged page fallback
            console.warn(`Could not render preview for damaged page ${i}`);
          }
        }
        ...
        console.warn("PDF preview rendering encountered corrupt structures:", err);
  ...
  const handleRepairPdf = async () => {
    ...
    setProcessing(true);
    setStatusMessage("Uploading document for repair... 0%");
    ...
    try {
      ...
      const responsePromise = new Promise<{ blob: Blob; fileName: string }>((resolve, reject) => {
        ...
        xhr.upload.addEventListener("load", () => {
          setStatusMessage("Analyzing cross-reference tables & streams...");
          let pct = 35;
          progressInterval = setInterval(() => {
            if (pct < 95) {
              ...
              if (pct < 55) {
                setStatusMessage("Rebuilding damaged XREF indexes & page trees...");
              } else if (pct < 80) {
                setStatusMessage("Defragmenting byte streams & embedded fonts...");
              } else {
                setStatusMessage("Finalizing repaired PDF document...");
              }
            }
          }, 300);
        });
        ...
        xhr.addEventListener("load", () => {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            const fileName =
              match?.[1] || `${file.name.replace(/\.[^/.]+$/, "")}_repaired.pdf`;
            resolve({ blob, fileName });
          } else {
            try {
              const err = JSON.parse(xhr.responseText);
              reject(new Error(err.error || "Repair failed"));
            } catch {
        ...
        xhr.addEventListener("abort", () => {
          cleanup();
          reject(new Error("Repair was cancelled"));
        });
      ...
      toast.success("PDF document successfully repaired and downloaded!");
    } catch (err) {
      console.error(err);
  ...
  const handleDownloadResultAgain = () => {
    ...
    a.click();
    URL.revokeObjectURL(url);
    toast.success("Downloaded repaired PDF!");
  };
      ...
      <div className="flex flex-col gap-6">
          ...
          <label className="upload-dropzone upload-dropzone-pdf w-full relative group cursor-pointer">
            ...
            <span className="text-center">
              <p className="text-base font-semibold text-zinc-800 dark:text-zinc-100">
                Click or drag a damaged PDF file here to repair
              </p>
              <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                Fix corrupted cross-reference tables, unreadable pages, broken streams, and unopenable PDFs.
              </p>
        ...
        {file && (
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
            {/* Left Area: Visual Document Inspection & Diagnostic */}
            <div className="lg:col-span-7 flex flex-col gap-4">
              {/* Header Document Diagnostic Card */}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5 text-xs text-zinc-500 dark:text-zinc-400">
                      ...
                      <span>•</span>
                      <span>{totalPages > 0 ? `${totalPages} Pages` : "Structure Damaged"}</span>
                      <span>•</span>
                      <span className="inline-flex items-center gap-1 text-amber-600 dark:text-amber-400 font-semibold">
                        <Activity className="h-3.5 w-3.5" /> Recoverable
                      </span>
                    </div>
                  </div>
                </div>
              ...
              {/* Repair Result Banner (Shown after conversion) */}
              {resultBlob && (
              ...
              {/* Thumbnails Gallery or Corruption Diagnostic Box */}
                ...
                {loadingPdf ? (
                  <div className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <p className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">
                      Analyzing damaged PDF structure...
                    </p>
                    <p className="text-xs text-zinc-500 mt-1">{loadingProgress}</p>
                  </div>
                ) : previewError || pages.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-16 text-center max-w-md mx-auto">
                    <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-amber-50 text-amber-600 dark:bg-amber-950/30 dark:text-amber-400 mb-4 shadow-xs">
                    ...
                    <p className="text-xs text-zinc-500 mt-1.5 leading-relaxed">
                      Standard browser PDF viewers cannot render this file due to broken cross-reference tables or damaged object pointers. Our server repair engine will reconstruct and salvage the contents.
                    </p>
                    <div className="mt-4 flex items-center gap-2 rounded-lg bg-zinc-100 px-3 py-1.5 text-[11px] font-semibold text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
                      <Wrench className="h-3.5 w-3.5 text-red-600" />
                      Ready for multi-tier structure recovery
                    </div>
                  </div>
                ) : (
                  <div
            ...
            {/* Right Area: Repair Strategy & Diagnostic Checklist */}
              ...
              <div className="sticky top-6 flex flex-col gap-5 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
                <div>
                  ...
                  <p className="text-xs text-zinc-500 mt-1">
                    Select a recovery method to salvage your damaged document.
                  </p>
                </div>
                ...
                {/* Diagnostic & Salvage Checklist Feature */}
                <div className="rounded-xl border border-zinc-200 bg-zinc-50/80 p-3.5 dark:border-zinc-800 dark:bg-zinc-900/50 space-y-2">
                  <span className="text-xs font-bold text-zinc-800 dark:text-zinc-200 flex items-center gap-1.5">
                    ...
                    Automated Structural Repair Passes:
                  </span>
                  <div className="grid grid-cols-1 gap-1.5 text-[11px] text-zinc-600 dark:text-zinc-400">
                    <div className="flex items-center gap-2">
                      <CheckCircle2 className="h-3 w-3 text-emerald-600 dark:text-emerald-400 shrink-0" />
                      <span>Cross-Reference (XREF) Table Reconstruction</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <CheckCircle2 className="h-3 w-3 text-emerald-600 dark:text-emerald-400 shrink-0" />
                      <span>Page Catalog & Tree Dictionary Repair</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <CheckCircle2 className="h-3 w-3 text-emerald-600 dark:text-emerald-400 shrink-0" />
                      <span>Byte Stream Defragmentation & Object Salvage</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <CheckCircle2 className="h-3 w-3 text-emerald-600 dark:text-emerald-400 shrink-0" />
                      <span>Linearization for Fast Web Viewing</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="border-t border-zinc-100 pt-3 dark:border-zinc-900 text-xs space-y-2.5">
                  ...
                  <div className="flex justify-between text-zinc-600 dark:text-zinc-400">
                    ...
                    <span className="font-bold text-red-600 dark:text-red-400">
                      Multi-Tier Structural Salvage (QPDF + GS)
                    </span>
                  </div>
                </div>
                ...
                >
                  {processing ? (
                    <>
                      ...
                      <span>{statusMessage || "Repairing PDF..."}</span>
                    </>
                  ) : (
                    <>
