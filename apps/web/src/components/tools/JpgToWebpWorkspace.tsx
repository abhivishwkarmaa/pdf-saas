...
interface JpgToWebpWorkspaceProps {
  tool: ToolDefinition;
}
...
export function JpgToWebpWorkspace({ tool }: JpgToWebpWorkspaceProps) {
  ...
  // Convert single JPG to WebP using browser canvas
  const convertSingleToWebp = (item: ConvertItem, qualityPercent: number): Promise<Blob> => {
    return new Promise((resolve, reject) => {
      ...
      img.onerror = () => reject(new Error("Failed to load JPG"));
  ...
  const handleConvertAll = async () => {
    ...
    setProcessing(true);
    setStatusMessage("Converting JPG photos to ultra-compact WebP... 0%");
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        const item = images[i];
        setStatusMessage(`Converting image ${i + 1} of ${images.length} to WebP...`);
        const convertedBlob = await convertSingleToWebp(item, quality);
      ...
      setImages(updated);
      setIsConverted(true);
      toast.success("All JPGs converted to WebP successfully!");
    } catch (err) {
      console.error(err);
  ...
  // Download single WebP
  const downloadSingleWebp = (item: ConvertItem) => {
  ...
  const downloadAllAsZip = async () => {
    ...
    if (images.length === 1 && images[0].convertedBlob) {
      downloadSingleWebp(images[0]);
      return;
    }

    setProcessing(true);
    setStatusMessage("Bundling converted WebP files into ZIP archive...");
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `converted_webp_${Date.now()}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);

      toast.success("Downloaded all WebP images as ZIP!");
    } catch (err) {
      console.error(err);
  ...
  const totalConvertedMb = (
    images.reduce((acc, img) => acc + (img.convertedSize || img.originalSize), 0) /
    (1024 * 1024)
  ).toFixed(2);
    ...
    <div className="mx-auto max-w-6xl px-3 sm:px-4 py-6 sm:py-10">
      ...
      {images.length === 0 && (
        <div className="mb-8 text-center">
          ...
          >
            ...
            Next-Gen JPG to WebP Compressor
          </span>
        ...
        <label className="upload-dropzone upload-dropzone-image w-full relative group cursor-pointer">
          ...
          <span className="text-center">
            <p className="text-base font-semibold text-zinc-800 dark:text-zinc-100">
              Click or drag JPG/JPEG photos here to convert to WebP
            </p>
            <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
              Cut image file size by up to 80% with next-generation WebP compression.
            </p>
      ...
      {images.length > 0 && (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
            ...
            {isConverted && (
              <div className="rounded-xl border border-emerald-200 bg-emerald-50/70 p-4 text-emerald-950 dark:border-emerald-900/50 dark:bg-emerald-950/20 dark:text-emerald-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-xs">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-emerald-800 dark:text-emerald-300 mt-0.5">
                      Reduced total size from {totalOriginalMb} MB to {totalConvertedMb} MB.
                    </p>
                  </div>
                </div>
                ...
                >
                  ...
                  {images.length > 1 ? "Download All (ZIP)" : "Download WebP"}
                </button>
              </div>
            )}
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-xs">
                      ...
                      <p className="text-[11px] text-zinc-400">
                        ...
                        {item.convertedSize && (
                          <span className="text-emerald-600 dark:text-emerald-400 font-bold ml-1 font-mono">
                            → {(item.convertedSize / 1024).toFixed(0)} KB WebP
                          </span>
                        )}
                      </p>
                    </div>
                    ...
                    {item.convertedBlob && (
                      <button
                        type="button"
                        onClick={() => downloadSingleWebp(item)}
          ...
          {/* Right Column: WebP Quality & Settings */}
            ...
            <div className="flex flex-col gap-5 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
              <div>
                ...
                <p className="text-xs text-zinc-500">
                  WebP delivers 30-50% smaller sizes than JPG at equivalent visual quality.
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-[10px] text-zinc-400">
                  <span>Lightweight (30%)</span>
                  <span>Recommended (85%)</span>
                  <span>Maximum (100%)</span>
                </div>
              </div>

              {/* WebP Advantage Points */}
              <div className="rounded-xl border border-zinc-200 bg-zinc-50/80 p-3.5 dark:border-zinc-800 dark:bg-zinc-900/50 space-y-2 text-xs text-zinc-600 dark:text-zinc-400">
                <div className="flex items-center gap-2">
                  ...
                  <span>Faster page loading speed on mobile devices</span>
                </div>
                <div className="flex items-center gap-2">
                  <Globe className="h-4 w-4 text-blue-500 shrink-0" />
                  <span>Supported by Chrome, Safari, Edge, Firefox & iOS</span>
                </div>
              </div>
              ...
              >
                {processing ? (
                  ...
                  <>
                    ...
                    <span>Convert {images.length} JPG{images.length === 1 ? "" : "s"} to WebP</span>
                  </>
                )}
              </button>
            </div>
