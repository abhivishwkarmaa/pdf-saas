...
interface HeicToJpgWorkspaceProps {
  tool: ToolDefinition;
}
...
export function HeicToJpgWorkspace({ tool }: HeicToJpgWorkspaceProps) {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    toast.success(`Added ${newItems.length} HEIC photo(s)!`);
  };
  ...
  // Convert single HEIC file via server API `/api/process/heic-to-jpg`
  const convertSingleHeic = async (item: ConvertItem): Promise<{ blob: Blob; url: string }> => {
    const formData = new FormData();
    formData.append("files", item.file);
    formData.append("options", JSON.stringify({ quality }));

    const res = await fetch(`/api/process/heic-to-jpg`, {
      method: "POST",
      body: formData,
    });
    ...
    if (!res.ok) {
      ...
      throw new Error(err.message || "Failed to convert HEIC to JPG");
    }
  ...
  // Batch convert all HEIC photos
  const handleConvertAll = async () => {
    if (images.length === 0) {
      toast.error("Please upload at least one HEIC/HEIF photo.");
      return;
    }

    setProcessing(true);
    setStatusMessage("Converting Apple HEIC photos to universal JPG... 0%");
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        const item = images[i];
        setStatusMessage(`Converting photo ${i + 1} of ${images.length} to JPG...`);
        const { blob, url } = await convertSingleHeic(item);
      ...
      setImages(updated);
      setIsConverted(true);
      toast.success("All HEIC photos converted to JPG successfully!");
    } catch (err: any) {
      console.error(err);
      toast.error(err.message || "Failed to convert one or more HEIC files.");
    } finally {
      setProcessing(false);
    }
  };
  ...
  const downloadSingleJpg = (item: ConvertItem) => {
    ...
    const url = item.previewUrl || URL.createObjectURL(item.convertedBlob);
  ...
  const downloadAllAsZip = async () => {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `converted_heic_jpgs_${Date.now()}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      ...
      {images.length === 0 && (
        <div className="mb-8 text-center">
          ...
          >
            ...
            iPhone HEIC / HEIF to JPG Converter
          </span>
        ...
        <label className="upload-dropzone upload-dropzone-image w-full relative group cursor-pointer">
          ...
          <span className="text-center">
            <p className="text-base font-semibold text-zinc-800 dark:text-zinc-100">
              Click or drag Apple iPhone HEIC/HEIF photos here
            </p>
            <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
              Convert high-efficiency Apple photos into universally compatible JPGs.
            </p>
            <p className="mt-2 text-[11px] font-medium text-zinc-400">
              Max {tool.maxFiles} images · Up to {tool.maxMb} MB each · Fast & Secure
            </p>
          </span>
        </label>
      )}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-zinc-900 dark:text-white">
                    {images.length} HEIC Photo{images.length === 1 ? "" : "s"} Loaded
                  </h3>
                  ...
                  <div className="w-full h-36 rounded-lg bg-zinc-100 dark:bg-zinc-900 overflow-hidden flex items-center justify-center relative">
                    {item.previewUrl ? (
                      ...
                      <div className="flex flex-col items-center gap-1.5 text-zinc-400">
                        ...
                        <span className="text-[11px] font-medium">Apple HEIC Photo</span>
                      </div>
                    )}
            ...
            <div className="flex flex-col gap-5 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
              <div>
                ...
                <p className="text-xs text-zinc-500">
                  Export iPhone photos into standard JPEG format with custom quality.
                </p>
              </div>
              ...
              <div className="rounded-xl border border-zinc-200 bg-zinc-50/80 p-3.5 dark:border-zinc-800 dark:bg-zinc-900/50 space-y-2 text-xs text-zinc-600 dark:text-zinc-400">
                <div className="flex items-center gap-2">
                  ...
                  <span>Opens on all Windows PC photo viewers</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="h-4 w-4 text-emerald-600 dark:text-emerald-400 shrink-0" />
                  <span>Supports upload to portals, job apps & forms</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="h-4 w-4 text-emerald-600 dark:text-emerald-400 shrink-0" />
                  <span>Preserves original EXIF & color vibrancy</span>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-zinc-200 bg-zinc-50/80 p-3.5 dark:border-zinc-800 dark:bg-zinc-900/50 space-y-1 text-[11px] text-zinc-600 dark:text-zinc-400">
                ...
                <p>
                  Photos are processed in isolated sandboxed workers and instantly deleted after download.
                </p>
              </div>
              ...
              >
                {processing ? (
                  ...
                  <>
                    ...
                    <span>Convert {images.length} HEIC{images.length === 1 ? "" : "s"} to JPG</span>
                  </>
                )}
              </button>
            </div>
