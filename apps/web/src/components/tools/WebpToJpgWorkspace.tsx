...
interface WebpToJpgWorkspaceProps {
  tool: ToolDefinition;
}
...
export function WebpToJpgWorkspace({ tool }: WebpToJpgWorkspaceProps) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    toast.success(`Added ${newItems.length} WebP image(s)!`);
  };
  ...
  // Convert single WebP to JPG using canvas
  const convertSingleWebpToJpg = (
    item: ConvertItem,
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        // Fill background color for transparent WebP
      ...
      img.onerror = () => reject(new Error("Failed to load source WebP"));
      img.src = item.previewUrl;
    });
  };

  // Batch convert all WebP to JPG
  const handleConvertAll = async () => {
    if (images.length === 0) {
      toast.error("Please upload at least one WebP image.");
      return;
    }

    setProcessing(true);
    setStatusMessage("Converting WebP images to standard JPG... 0%");
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        const convertedBlob = await convertSingleWebpToJpg(item, quality, bgFill);
      ...
      setImages(updated);
      setIsConverted(true);
      toast.success("All WebP images converted to JPG successfully!");
    } catch (err) {
      console.error(err);
      toast.error("Failed to convert one or more WebP images.");
    } finally {
      setProcessing(false);
    }
  };
      ...
      {images.length === 0 && (
        <div className="mb-8 text-center">
          ...
          >
            ...
            Universal WebP to JPG Converter
          </span>
        ...
        <label className="upload-dropzone upload-dropzone-image w-full relative group cursor-pointer">
          ...
          <span className="text-center">
            <p className="text-base font-semibold text-zinc-800 dark:text-zinc-100">
              Click or drag WebP images here to convert to JPG
            </p>
            <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
              Convert web-optimized WebP files into universally compatible JPG photos.
            </p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-zinc-900 dark:text-white">
                    {images.length} WebP Image{images.length === 1 ? "" : "s"} Loaded
                  </h3>
            ...
            <div className="flex flex-col gap-5 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
              <div>
                ...
                <p className="text-xs text-zinc-500">
                  Configure output quality and background fill for transparent WebP images.
                </p>
              </div>
              ...
              >
                {processing ? (
                  ...
                  <>
                    ...
                    <span>Convert {images.length} WebP{images.length === 1 ? "" : "s"} to JPG</span>
                  </>
                )}
              </button>
            </div>
