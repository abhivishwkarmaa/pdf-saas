...
interface BmpToJpgWorkspaceProps {
  tool: ToolDefinition;
}
...
export function BmpToJpgWorkspace({ tool }: BmpToJpgWorkspaceProps) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    toast.success(`Added ${newItems.length} BMP image(s)!`);
  };
  ...
  // Convert single BMP to JPG using browser canvas
  const convertSingleBmpToJpg = (item: ConvertItem, qualityPercent: number): Promise<Blob> => {
      ...
      img.onload = () => {
        ...
        canvas.toBlob(
          (blob) => {
            ...
            else reject(new Error("BMP conversion failed"));
          },
          "image/jpeg",
          qualityPercent / 100
        );
      };
      img.onerror = () => reject(new Error("Failed to load source BMP"));
      img.src = item.previewUrl;
    });
  };

  // Batch convert all BMP to JPG
  const handleConvertAll = async () => {
    if (images.length === 0) {
      toast.error("Please upload at least one BMP image.");
      return;
    }

    setProcessing(true);
    setStatusMessage("Compressing heavy BMP files into lightweight JPG... 0%");
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        const convertedBlob = await convertSingleBmpToJpg(item, quality);
      ...
      setImages(updated);
      setIsConverted(true);
      toast.success("All BMP images converted to JPG successfully!");
    } catch (err) {
      console.error(err);
      toast.error("Failed to convert one or more BMP images.");
    } finally {
      setProcessing(false);
    }
  };
  ...
  const downloadAllAsZip = async () => {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `converted_bmp_jpgs_${Date.now()}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      ...
      {images.length === 0 && (
        <div className="mb-8 text-center">
          ...
          >
            ...
            Bitmap (BMP) to JPG Compressor & Converter
          </span>
        ...
        <label className="upload-dropzone upload-dropzone-image w-full relative group cursor-pointer">
          ...
          <span className="text-center">
            <p className="text-base font-semibold text-zinc-800 dark:text-zinc-100">
              Click or drag Windows Bitmap (.bmp) files here
            </p>
            <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
              Convert raw uncompressed bitmaps into lightweight universal JPG photos.
            </p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-zinc-900 dark:text-white">
                    {images.length} BMP File{images.length === 1 ? "" : "s"} Loaded
                  </h3>
            ...
            <div className="flex flex-col gap-5 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
              <div>
                ...
                <p className="text-xs text-zinc-500">
                  Configure output quality to maximize file size reduction.
                </p>
              </div>
              ...
              >
                {processing ? (
                  <>
                    ...
                    <span>Converting BMP to JPG...</span>
                  </>
                ) : (
                  <>
                    <FileType className="h-4 w-4" />
                    <span>Convert {images.length} BMP{images.length === 1 ? "" : "s"} to JPG</span>
                  </>
                )}
              </button>
            </div>
