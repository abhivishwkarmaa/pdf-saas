...
const STRATEGY_PRESETS: StrategyPreset[] = [
  {
    ...
    description: "Automatically analyzes document structure, font glyphs, and tables to apply the optimal layout reconstruction engine.",
    badge: "Recommended",
    recommendedFor: "General PDF documents, contracts, reports & forms",
    isPopular: true,
  },
  {
    id: "local",
    title: "Text & Flowing Content (Fast)",
    description: "Fast conversion optimized for standard text paragraphs, essays, articles, and documents with minimal complex tables.",
    badge: "Fast & Clean",
    recommendedFor: "Articles, books, clean text documents & letters",
  },
  {
    id: "adobe",
    title: "Tables & Complex Graphics (Enhanced)",
    description: "Prioritizes precise table cell grids, multi-column layouts, header/footer zones, and embedded graphic positioning.",
    badge: "Best for Tables",
    recommendedFor: "Financial sheets, invoices, multi-column newsletters & resumes",
  },
];

export function PdfToWordWorkspace({ tool }: PdfToWordWorkspaceProps) {
  ...
  useEffect(() => {
    ...
    const renderPdfPages = async () => {
      setLoadingPdf(true);
      setLoadingProgress("Analyzing PDF pages...");
      setPages([]);
      setResultBlob(null);
    ...
    try {
      ...
      const responsePromise = new Promise<{ blob: Blob; fileName: string }>((resolve, reject) => {
        ...
        xhr.upload.addEventListener("load", () => {
          setStatusMessage("Analyzing layout & paragraph structure...");
          let pct = 30;
          progressInterval = setInterval(() => {
            if (pct < 95) {
              ...
              if (pct < 55) {
                setStatusMessage("Extracting text formatting, fonts & alignments...");
              } else if (pct < 80) {
                setStatusMessage("Rebuilding tables, cell grids & images...");
              } else {
                setStatusMessage("Finalizing Microsoft Word (.docx) document...");
              }
            }
          }, 320);
        });
        ...
        xhr.addEventListener("load", () => {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            const fileName =
              match?.[1] || `${file.name.replace(/\.[^/.]+$/, "")}.docx`;
            resolve({ blob, fileName });
          } else {
            try {
              const err = JSON.parse(xhr.responseText);
              reject(new Error(err.error || "Word conversion failed"));
            } catch {
              reject(new Error("Word conversion failed"));
            }
          }
        });
      ...
      toast.success("PDF successfully converted to editable Word document (.docx)!");
    } catch (err) {
      console.error(err);
      ...
      <div className="flex flex-col gap-6">
        ...
        {!file && (
          <label className="upload-dropzone upload-dropzone-pdf w-full relative group cursor-pointer">
            ...
            <span className="upload-icon-container group-hover:scale-105 transition-transform">
              <FileEdit className="h-8 w-8 text-red-600 dark:text-red-400" />
            </span>
            <span className="text-center">
              <p className="text-base font-semibold text-zinc-800 dark:text-zinc-100">
                Click or drag a PDF file here to convert to Word (DOCX)
              </p>
              <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                Convert PDF to an editable Microsoft Word document with full text, table, and image preservation.
              </p>
        ...
        {file && (
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5 text-xs text-zinc-500 dark:text-zinc-400">
                      ...
                      <span className="inline-flex items-center gap-1 text-blue-600 dark:text-blue-400 font-semibold">
                        <FileEdit className="h-3.5 w-3.5" /> Output: .DOCX
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-emerald-800 dark:text-emerald-300 mt-0.5">
                        Editable text, layout & tables preserved successfully.
                      </p>
                    </div>
                  </div>
            ...
            {/* Right Area: Conversion Strategy & Document Checklist */}
              ...
              <div className="sticky top-6 flex flex-col gap-5 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
                <div>
                  ...
                  <p className="text-xs text-zinc-500 mt-1">
                    Select a layout reconstruction mode for your Word document.
                  </p>
                </div>
                ...
                <div className="rounded-xl border border-zinc-200 bg-zinc-50/80 p-3.5 dark:border-zinc-800 dark:bg-zinc-900/50 space-y-2">
                  ...
                  <div className="grid grid-cols-1 gap-1.5 text-[11px] text-zinc-600 dark:text-zinc-400">
                    <div className="flex items-center gap-2">
                      ...
                      <span>100% Editable Paragraphs & Text Flow</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <CheckCircle2 className="h-3 w-3 text-emerald-600 dark:text-emerald-400 shrink-0" />
                      <span>Table Gridlines & Cell Formatting Preserved</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <CheckCircle2 className="h-3 w-3 text-emerald-600 dark:text-emerald-400 shrink-0" />
                      <span>Embedded High-Resolution Images Retained</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <CheckCircle2 className="h-3 w-3 text-emerald-600 dark:text-emerald-400 shrink-0" />
                      <span>Font Styling, Bold, Italics & Alignment</span>
                    </div>
                  </div>
                </div>
                ...
                <button
                  onClick={() => void handleConvertToWord()}
                  disabled={processing}
                  className={cn(
                ...
                >
                  {processing ? (
                    <>
                      ...
                      <span>{statusMessage || "Converting to Word..."}</span>
                    </>
                  ) : (
                    <>
                ...
                {processing && (
                  <div className="space-y-1">
                    ...
                    <div className="flex justify-between text-[10px] text-zinc-400">
                      <span>DOCX layout construction</span>
