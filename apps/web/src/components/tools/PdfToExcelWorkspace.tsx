...
interface PdfToExcelWorkspaceProps {
  tool: ToolDefinition;
}
...
type ExtractionStrategy = "auto" | "columns" | "clean";
...
const STRATEGY_PRESETS: StrategyPreset[] = [
  {
    id: "auto",
    title: "Smart Table Grid (Recommended)",
    description: "Automatically detects table cell borders, headers, rows, and columns for accurate multi-table extraction.",
    badge: "Recommended",
    recommendedFor: "Invoices, general financial reports & bank statements",
    isPopular: true,
  },
  {
    id: "columns",
    title: "Exact Column Alignment",
    description: "Maintains wide columnar positioning for documents with borderless tabular data and wide numerical charts.",
    badge: "Wide Data",
    recommendedFor: "Balance sheets, ledger reports & wide data grids",
  },
  {
    id: "clean",
    title: "Clean Raw Data (Formula-Ready)",
    description: "Strips decorative formatting and delivers clean, formula-ready rows optimized for PivotTables and data analysis.",
    badge: "Analytics",
    recommendedFor: "Raw data exports, statistical figures & CSV analysis",
  },
];

export function PdfToExcelWorkspace({ tool }: PdfToExcelWorkspaceProps) {
  ...
  const [selectedStrategy, setSelectedStrategy] = useState<ExtractionStrategy>("auto");
  const [autoDetectNumbers, setAutoDetectNumbers] = useState<boolean>(true);
  ...
  useEffect(() => {
    ...
    const renderPdfPages = async () => {
      setLoadingPdf(true);
      setLoadingProgress("Analyzing tables...");
      setPages([]);
      setResultBlob(null);
    ...
    try {
      ...
      formData.append(
        ...
        JSON.stringify({
          ...
          detectNumbers: autoDetectNumbers,
        })
      );
      ...
      const responsePromise = new Promise<{ blob: Blob; fileName: string }>((resolve, reject) => {
        ...
        xhr.upload.addEventListener("load", () => {
          setStatusMessage("Analyzing table structures & grids...");
          let pct = 30;
          progressInterval = setInterval(() => {
            if (pct < 95) {
              ...
              if (pct < 55) {
                setStatusMessage("Extracting table rows, columns & headers...");
              } else if (pct < 80) {
                setStatusMessage("Parsing numerical fields & cell formats...");
              } else {
                setStatusMessage("Compiling Microsoft Excel (.xlsx) spreadsheet...");
              }
            }
          }, 320);
        });
        ...
        xhr.addEventListener("load", () => {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            const fileName =
              match?.[1] || `${file.name.replace(/\.[^/.]+$/, "")}.xlsx`;
            resolve({ blob, fileName });
          } else {
            try {
              const err = JSON.parse(xhr.responseText);
              reject(new Error(err.error || "Excel conversion failed"));
            } catch {
              reject(new Error("Excel conversion failed"));
            }
          }
        });
      ...
      toast.success("PDF successfully converted to Excel spreadsheet (.xlsx)!");
    } catch (err) {
      console.error(err);
      ...
      <div className="flex flex-col gap-6">
          ...
          <label className="upload-dropzone upload-dropzone-pdf w-full relative group cursor-pointer">
            ...
            <span className="text-center">
              <p className="text-base font-semibold text-zinc-800 dark:text-zinc-100">
                Click or drag a PDF file here to convert to Excel (XLSX)
              </p>
              <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                Extract tables and structured data directly into editable Microsoft Excel spreadsheets.
              </p>
        ...
        {file && (
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5 text-xs text-zinc-500 dark:text-zinc-400">
                      ...
                      <span className="inline-flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-semibold">
                        <FileSpreadsheet className="h-3.5 w-3.5" /> Output: .XLSX
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-emerald-800 dark:text-emerald-300 mt-0.5">
                        Extracted tables, numerical values & formulas are ready for analysis.
                      </p>
                    </div>
                  </div>
            ...
            {/* Right Area: Spreadsheet Extraction Strategy & Options */}
              ...
              <div className="sticky top-6 flex flex-col gap-5 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
                <div>
                  ...
                  <p className="text-xs text-zinc-500 mt-1">
                    Select how tables and structured data should be extracted.
                  </p>
                </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-2 flex items-center justify-between border-t border-zinc-100 pt-2 pl-6 text-[11px] text-zinc-500 dark:border-zinc-800 dark:text-zinc-400">
                          <span className="truncate">Best For: {preset.recommendedFor}</span>
                        </div>
                      </div>
                    );
                  })}
                ...
                <div className="border-t border-zinc-100 pt-3 dark:border-zinc-900 text-xs">
                  <label className="flex items-center justify-between cursor-pointer">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold text-zinc-700 dark:text-zinc-300">
                        Parse Numbers & Currency Format
                      </span>
                    </div>
                    <input
                      type="checkbox"
                      checked={autoDetectNumbers}
                      onChange={(e) => setAutoDetectNumbers(e.target.checked)}
                      className="h-4 w-4 rounded border-zinc-300 text-red-600 focus:ring-red-500 dark:border-zinc-700 dark:bg-zinc-800"
                    />
                  </label>
                </div>

                {/* Excel Features Checklist */}
                <div className="rounded-xl border border-zinc-200 bg-zinc-50/80 p-3.5 dark:border-zinc-800 dark:bg-zinc-900/50 space-y-2">
                  ...
                  <div className="grid grid-cols-1 gap-1.5 text-[11px] text-zinc-600 dark:text-zinc-400">
                    <div className="flex items-center gap-2">
                      ...
                      <span>Native Microsoft Excel (.xlsx) format</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <CheckCircle2 className="h-3 w-3 text-emerald-600 dark:text-emerald-400 shrink-0" />
                      <span>Auto-detects numeric values for SUM/AVERAGE formulas</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <CheckCircle2 className="h-3 w-3 text-emerald-600 dark:text-emerald-400 shrink-0" />
                      <span>Clean row/column tabular boundaries</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <CheckCircle2 className="h-3 w-3 text-emerald-600 dark:text-emerald-400 shrink-0" />
                      <span>Compatible with Excel, Google Sheets & Calc</span>
                    </div>
                  </div>
                </div>
                ...
                <button
                  onClick={() => void handleConvertToExcel()}
                  disabled={processing}
                  className={cn(
                ...
                >
                  {processing ? (
                    <>
                      ...
                      <span>{statusMessage || "Extracting tables to Excel..."}</span>
                    </>
                  ) : (
                    <>
                ...
                {processing && (
                  <div className="space-y-1">
                    ...
                    <div className="flex justify-between text-[10px] text-zinc-400">
                      <span>Spreadsheet table extraction</span>
