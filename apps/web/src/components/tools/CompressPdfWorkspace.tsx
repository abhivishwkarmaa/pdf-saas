...
type CompressionLevel = "screen" | "ebook" | "print";
...
const COMPRESSION_PRESETS: CompressionPreset[] = [
  {
    ...
    description: "Maximum file reduction with low resolution (72 DPI). Ideal for quick email attachments and web sharing.",
    dpi: "72 DPI",
    estimatedReduction: "~70% - 80% smaller",
    badge: "Smallest Size",
  },
  {
    id: "ebook",
    title: "Recommended Compression",
    description: "Balanced optimization with standard resolution (150 DPI). Perfect balance of crisp text, clear images, and compact size.",
    dpi: "150 DPI",
    estimatedReduction: "~40% - 60% smaller",
    badge: "Recommended",
    isPopular: true,
  },
  {
    id: "print",
    title: "Less Compression (High Quality)",
    description: "High print resolution (300 DPI) while compacting streams and removing bloat metadata. Best for high-res printing.",
    dpi: "300 DPI",
    estimatedReduction: "~20% - 35% smaller",
    badge: "Best Quality",
  },
];

export function CompressPdfWorkspace({ tool }: CompressPdfWorkspaceProps) {
  ...
  const handleCompress = async () => {
    ...
    try {
      ...
      const responsePromise = new Promise<{ blob: Blob; fileName: string }>((resolve, reject) => {
        xhr.upload.addEventListener("progress", (event) => {
          if (event.lengthComputable) {
            ...
            setProgressPercent(Math.min(40, Math.round(percent * 0.4)));
          }
        });

        xhr.upload.addEventListener("load", () => {
          setStatusMessage("Optimizing streams & images...");
          let pct = 40;
          progressInterval = setInterval(() => {
            if (pct < 95) {
              ...
              if (pct < 60) {
                setStatusMessage("Optimizing font & vector assets...");
              } else if (pct < 85) {
                setStatusMessage("Compressing embedded image objects...");
              } else {
                setStatusMessage("Finalizing compacted PDF...");
              }
            }
          }, 300);
        });
        ...
        xhr.addEventListener("load", () => {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            const fileName =
              match?.[1] || `${file.name.replace(/\.[^/.]+$/, "")}_compressed.pdf`;
            resolve({ blob, fileName });
          } else {
            try {
              const err = JSON.parse(xhr.responseText);
              reject(new Error(err.error || "Compression failed"));
            } catch {
      ...
      toast.success(
        savings > 0
          ? `Success! File reduced by ${savings}% (${(file.size / (1024 * 1024)).toFixed(2)}MB → ${(blob.size / (1024 * 1024)).toFixed(2)}MB)`
  ...
  const handleDownloadResultAgain = () => {
    ...
    a.click();
    URL.revokeObjectURL(url);
    toast.success("Downloaded compressed PDF!");
  };

  const originalSizeMb = file ? (file.size / (1024 * 1024)).toFixed(2) : "0";
  const compressedSizeMb = compressedSize ? (compressedSize / (1024 * 1024)).toFixed(2) : "0";
        ...
        {!file && (
          <label className="upload-dropzone upload-dropzone-pdf w-full relative group cursor-pointer">
            ...
            <span className="upload-icon-container group-hover:scale-105 transition-transform">
              <Minimize2 className="h-8 w-8 text-red-600 dark:text-red-400" />
            </span>
            <span className="text-center">
              <p className="text-base font-semibold text-zinc-800 dark:text-zinc-100">
                Click or drag a PDF file here to compress
              </p>
              <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                Reduce file size significantly while preserving top visual quality and formatting.
              </p>
        ...
        {file && (
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
            ...
            <div className="lg:col-span-7 flex flex-col gap-4">
              ...
              {/* Compression Result Banner (Shown after conversion) */}
              {resultBlob && (
                      ...
                      <h4 className="text-sm font-bold flex items-center gap-2">
                        ...
                        {percentageSavings > 0 && (
                          <span className="rounded-full bg-emerald-600 px-2 py-0.5 text-[10px] font-black text-white uppercase">
                            -{percentageSavings}% Smaller
                          </span>
                        )}
                      </h4>
                      <p className="text-xs text-emerald-800 dark:text-emerald-300 mt-0.5">
                        {originalSizeMb} MB → <span className="font-bold">{compressedSizeMb} MB</span>
                      </p>
                    </div>
                  </div>
            ...
            {/* Right Area: Compression Level Selector & Actions */}
              ...
              <div className="sticky top-6 flex flex-col gap-5 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
                <div>
                  ...
                  <p className="text-xs text-zinc-500 mt-1">
                    Choose the best balance between file size and visual resolution.
                  </p>
                </div>

                {/* Compression Level Radio Cards */}
                  ...
                  {COMPRESSION_PRESETS.map((preset) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-2.5 flex items-center justify-between border-t border-zinc-100 pt-2 pl-6 text-[11px] font-semibold text-zinc-600 dark:border-zinc-800 dark:text-zinc-400">
                          <span>Target: {preset.dpi}</span>
                ...
                <div className="border-t border-zinc-100 pt-3 dark:border-zinc-900 text-xs space-y-2.5">
                  ...
                  <div className="flex justify-between text-zinc-600 dark:text-zinc-400">
                    ...
                    <span className="font-bold text-red-600 dark:text-red-400">
                      {COMPRESSION_PRESETS.find((p) => p.id === compressionLevel)?.title}
                    </span>
                  </div>
                  <div className="flex justify-between text-zinc-600 dark:text-zinc-400">
                    <span>Expected Output:</span>
                    <span className="font-semibold text-emerald-600 dark:text-emerald-400">
                      {COMPRESSION_PRESETS.find((p) => p.id === compressionLevel)?.estimatedReduction}
                    </span>
                  </div>
                </div>
