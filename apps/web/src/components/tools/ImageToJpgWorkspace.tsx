...
interface ImageToJpgWorkspaceProps {
  tool: ToolDefinition;
}
...
export function ImageToJpgWorkspace({ tool }: ImageToJpgWorkspaceProps) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const f of files) {
      ...
      const ext = f.name.split(".").pop()?.toUpperCase() || "IMAGE";
  ...
  // Convert single image to JPG using browser canvas
  const convertSingleToJpg = (
    item: ConvertItem,
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        // Fill background color for transparent PNGs/SVGs
  ...
  const handleConvertAll = async () => {
    ...
    setProcessing(true);
    setStatusMessage("Converting images to standard JPG... 0%");
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        const convertedBlob = await convertSingleToJpg(item, quality, bgFill);
      ...
      setImages(updated);
      setIsConverted(true);
      toast.success("All images converted to JPG successfully!");
    } catch (err) {
      console.error(err);
      toast.error("Failed to convert one or more images to JPG.");
    } finally {
      setProcessing(false);
    }
  };
  ...
  const downloadAllAsZip = async () => {
    ...
    setProcessing(true);
    setStatusMessage("Bundling converted JPGs into ZIP archive...");
      ...
      {images.length === 0 && (
        <label className="upload-dropzone upload-dropzone-image w-full relative group cursor-pointer">
          <input
            type="file"
            multiple
            accept="image/*,.heic,.heif,.svg,.avif,.webp,.png,.bmp,.gif,.tiff,.tif"
          ...
          <span className="text-center">
            <p className="text-base font-semibold text-zinc-800 dark:text-zinc-100">
              Click or drag any image format to convert to JPG
            </p>
            <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
              Supports PNG, WebP, HEIC, BMP, GIF, AVIF, TIFF, and SVG images.
            </p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-zinc-900 dark:text-white">
                    {images.length} Image{images.length === 1 ? "" : "s"} to Convert
                  </h3>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-emerald-800 dark:text-emerald-300 mt-0.5">
                      Compatible with all standard photo viewers, browsers, and printers.
                    </p>
                  </div>
                </div>
            ...
            <div className="flex flex-col gap-5 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
              <div>
                ...
                <p className="text-xs text-zinc-500">
                  Configure output quality and background fill for transparent images.
                </p>
              </div>
              ...
              {/* Background Color Fill for Transparent Images */}
              <div className="space-y-2 pt-2 border-t border-zinc-100 dark:border-zinc-800">
                ...
                <p className="text-[11px] text-zinc-400">
                  JPG does not support transparency. Select fill color for transparent areas.
                </p>
              ...
              {/* Animated GIF Info Box */}
              <div className="rounded-xl border border-blue-200 bg-blue-50/60 p-3 dark:border-blue-900/40 dark:bg-blue-950/20 text-[11px] text-blue-950 dark:text-blue-200 space-y-0.5">
                <span className="font-bold flex items-center gap-1.5">
                  <Sparkles className="h-3.5 w-3.5 text-blue-600 dark:text-blue-400" />
                  Animated GIFs:
                </span>
                <p className="text-zinc-600 dark:text-zinc-400">
                  Standard JPEG / JPG is a static photo format (does not support animations). Converting an animated GIF to JPG exports the high-resolution first still frame.
                </p>
              </div>
              ...
              <div className="rounded-xl border border-zinc-200 bg-zinc-50/80 p-3.5 dark:border-zinc-800 dark:bg-zinc-900/50 space-y-1 text-[11px] text-zinc-600 dark:text-zinc-400">
                ...
                <p>
                  Images are converted directly in your browser canvas engine. No files are uploaded to any server.
                </p>
              </div>
              ...
              >
                {processing ? (
                  ...
                  <>
                    ...
                    <span>Convert {images.length} Image{images.length === 1 ? "" : "s"} to JPG</span>
                  </>
                )}
              </button>
            </div>
