...
export function ExtractPagesWorkspace({ tool }: ExtractPagesWorkspaceProps) {
  ...
  // Set of selected page numbers (1-based) to EXTRACT
  ...
  const handleExtractPages = async () => {
    ...
    try {
      ...
      if (mergeIntoOne) {
        setStatusMessage("Generating merged extracted PDF...");
        ...
        toast.success(`Successfully extracted ${sortedPages.length} page(s) into 1 PDF!`);
      } else {
        if (sortedPages.length === 1) {
          ...
          setStatusMessage("Extracting individual pages into ZIP archive...");
          const zip = new JSZip();

          for (let i = 0; i < sortedPages.length; i++) {
            const pageNum = sortedPages[i];
            setStatusMessage(`Extracting page ${pageNum} (${i + 1}/${sortedPages.length})...`);
            const newDoc = await PDFDocument.create();
            const [copied] = await newDoc.copyPages(sourceDoc, [pageNum - 1]);
            newDoc.addPage(copied);
            const pdfBytes = await newDoc.save();
            const blob = new Blob([pdfBytes as any], { type: "application/pdf" });
            zip.file(`${baseName}_page_${pageNum}.pdf`, blob);
          }
          ...
          toast.success(`Downloaded ${sortedPages.length} extracted pages in a ZIP archive!`);
        }
      }
    } catch (err) {
          ...
          <label className="upload-dropzone upload-dropzone-pdf w-full relative group cursor-pointer">
            ...
            <span className="text-center">
              <p className="text-base font-semibold text-zinc-800 dark:text-zinc-100">
                Click or drag a PDF file here to extract pages
              </p>
              <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                Preview pages in real-time, select required pages with one click, and export as PDF or ZIP.
              </p>
            ...
            <div className="lg:col-span-8 flex flex-col gap-4">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-red-200 bg-red-50/70 p-3 text-xs dark:border-red-900/50 dark:bg-red-950/20">
                <div className="flex items-center gap-2 text-red-900 dark:text-red-200 font-medium">
                  ...
                  <span>Click thumbnails to select pages for extraction</span>
                </div>
              ...
              <div className="sticky top-6 flex flex-col gap-5 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
                <div>
                  ...
                  <p className="text-xs text-zinc-500 mt-1">
                    Extract selected pages into a new PDF document or ZIP.
                  </p>
                </div>
                    ...
                    <label className="flex items-start gap-2 cursor-pointer">
                      ...
                      <div className="text-xs">
                        ...
                        <p className="text-[11px] text-zinc-500">
                          Creates a single PDF file with the selected pages.
                        </p>
                      </div>
                    </label>
                ...
                <div className="border-t border-zinc-100 pt-3 dark:border-zinc-900 text-xs space-y-2.5">
                  ...
                  <div className="flex justify-between text-red-600 dark:text-red-400 font-bold">
                    ...
                    <span>{selectedPages.size} pages</span>
                  </div>
                  <div className="flex justify-between text-zinc-600 dark:text-zinc-400">
                ...
                {/* Action Button */}
                <button
                  onClick={() => void handleExtractPages()}
                  disabled={processing || totalPages === 0 || selectedPages.size === 0}
                  className={cn(
                    ...
                    theme.button,
                    (processing || selectedPages.size === 0) && "opacity-60 cursor-not-allowed"
                  )}
                >
                  {processing ? (
                    <>
                    ...
                    <>
                      ...
                      <span>Extract ({selectedPages.size}) Pages</span>
                    </>
                  )}
                </button>
              </div>
