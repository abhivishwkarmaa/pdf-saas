...
interface JpgToImageWorkspaceProps {
  tool: ToolDefinition;
}

interface TargetFormatOption {
  ext: "png" | "webp" | "bmp" | "gif" | "avif";
  name: string;
  desc: string;
  badge: string;
}

const TARGET_FORMATS: TargetFormatOption[] = [
  { ext: "png", name: "PNG", desc: "Lossless crisp format, great for graphics & text", badge: "Lossless" },
  { ext: "webp", name: "WebP", desc: "Next-gen ultra compact format for web & mobile", badge: "Next-Gen" },
  { ext: "bmp", name: "BMP", desc: "Uncompressed raw bitmap for Windows & print", badge: "Bitmap" },
  { ext: "gif", name: "GIF", desc: "Standard web graphic format (8-bit palette)", badge: "Graphic" },
  { ext: "avif", name: "AVIF", desc: "Modern AV1-based high efficiency image format", badge: "AV1" },
];
...
export function JpgToImageWorkspace({ tool }: JpgToImageWorkspaceProps) {
  ...
  const [targetExt, setTargetExt] = useState<"png" | "webp" | "bmp" | "gif" | "avif">("png");
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    toast.success(`Added ${newItems.length} JPG photo(s)!`);
  };
  ...
  // Convert single JPG to target format using canvas
  const convertSingleJpg = (
    item: ConvertItem,
    ext: "png" | "webp" | "bmp" | "gif" | "avif",
        ...
        canvas.toBlob(
          (blob) => {
            if (blob) resolve(blob);
              ...
              // Fallback to PNG if browser doesn't natively encode AVIF/BMP
              canvas.toBlob(
                (fallbackBlob) => {
          ...
          mimeType,
          ext === "webp" || ext === "avif" ? qualityPercent / 100 : undefined
        );
      };
      img.onerror = () => reject(new Error("Failed to load JPG image"));
  ...
  const handleConvertAll = async () => {
    if (images.length === 0) {
      toast.error("Please upload at least one JPG photo.");
      return;
    }

    setProcessing(true);
    setStatusMessage(`Converting JPGs to ${targetExt.toUpperCase()}... 0%`);
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        const item = images[i];
        setStatusMessage(`Converting image ${i + 1} of ${images.length} to ${targetExt.toUpperCase()}...`);
        const convertedBlob = await convertSingleJpg(item, targetExt, quality);
      ...
      setImages(updated);
      setIsConverted(true);
      toast.success(`All images converted to ${targetExt.toUpperCase()} successfully!`);
    } catch (err) {
      console.error(err);
      toast.error("Failed to convert one or more JPG images.");
    } finally {
      setProcessing(false);
    }
  };
  ...
  const downloadSingleImage = (item: ConvertItem) => {
  ...
  const downloadAllAsZip = async () => {
    ...
    setProcessing(true);
    setStatusMessage(`Bundling converted ${targetExt.toUpperCase()} files into ZIP...`);
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        if (item.convertedBlob) {
          const baseName = item.file.name.replace(/\.[^/.]+$/, "");
          zip.file(`${baseName}.${targetExt}`, item.convertedBlob);
        }
      }
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `converted_${targetExt}_${Date.now()}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);

      toast.success(`Downloaded all ${targetExt.toUpperCase()} files as ZIP!`);
    } catch (err) {
      console.error(err);
    ...
    <div className="mx-auto max-w-6xl px-3 sm:px-4 py-6 sm:py-10">
      ...
      {images.length === 0 && (
        <div className="mb-8 text-center">
          ...
          >
            ...
            Universal JPG to Multi-Format Converter
          </span>
        ...
        <label className="upload-dropzone upload-dropzone-image w-full relative group cursor-pointer">
          ...
          <span className="text-center">
            <p className="text-base font-semibold text-zinc-800 dark:text-zinc-100">
              Click or drag JPG/JPEG photos here to convert
            </p>
            <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
              Convert into PNG (Lossless), WebP (Compact), BMP, GIF, and AVIF formats.
            </p>
      ...
      {images.length > 0 && (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-zinc-900 dark:text-white">
                    {images.length} JPG Photo{images.length === 1 ? "" : "s"} Loaded
                  </h3>
            ...
            {isConverted && (
              <div className="rounded-xl border border-emerald-200 bg-emerald-50/70 p-4 text-emerald-950 dark:border-emerald-900/50 dark:bg-emerald-950/20 dark:text-emerald-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-xs">
                ...
                >
                  ...
                  {images.length > 1 ? `Download All (${targetExt.toUpperCase()})` : "Download Image"}
                </button>
              </div>
            )}
                  ...
                  <div className="flex items-center justify-between text-xs">
                      ...
                      <p className="text-[11px] text-zinc-400">
                        ...
                        {item.convertedSize && (
                          <span className="text-emerald-600 dark:text-emerald-400 font-bold ml-1 font-mono">
                            → {(item.convertedSize / 1024).toFixed(0)} KB {targetExt.toUpperCase()}
                          </span>
                        )}
                      </p>
                    </div>
                    ...
                    {item.convertedBlob && (
                      <button
                        ...
                        title={`Download ${targetExt.toUpperCase()}`}
                      >
                        <Download className="h-3.5 w-3.5" />
                        <span>Save</span>
                      </button>
          ...
          {/* Right Column: Target Format & Quality Settings */}
            ...
            <div className="flex flex-col gap-5 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
              <div>
                ...
                <p className="text-xs text-zinc-500">
                  Choose the destination file format for your JPG photos.
                </p>
              </div>
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold font-mono uppercase">{fmt.name}</span>
                        <span className="text-[10px] bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 px-1.5 py-0.5 rounded font-semibold">
                          {fmt.badge}
                        </span>
                      </div>
                      <p className="text-[11px] text-zinc-400 mt-0.5">{fmt.desc}</p>
                    </div>
                    {targetExt === fmt.ext && <Check className="h-4 w-4 text-blue-600 shrink-0" />}
                  </button>
                ))}
              </div>

              {/* Quality Slider (For WebP / AVIF) */}
              {(targetExt === "webp" || targetExt === "avif") && (
              ...
              >
                {processing ? (
                  <>
                    ...
                    <span>Converting to {targetExt.toUpperCase()}...</span>
                  </>
                ) : (
                  <>
                    <FileType className="h-4 w-4" />
                    <span>Convert {images.length} JPG{images.length === 1 ? "" : "s"} to {targetExt.toUpperCase()}</span>
                  </>
                )}
              </button>
            </div>
