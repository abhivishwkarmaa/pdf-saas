...
interface SvgToJpgWorkspaceProps {
  tool: ToolDefinition;
}
...
export function SvgToJpgWorkspace({ tool }: SvgToJpgWorkspaceProps) {
  ...
  const [scale, setScale] = useState<number>(2); // 1x, 2x (HD), 4x (Ultra HD)
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    toast.success(`Added ${newItems.length} SVG graphic(s)!`);
  };
  ...
  // Convert single SVG to JPG using canvas with high resolution scale factor
  const convertSingleSvgToJpg = (
    item: ConvertItem,
    renderScale: number,
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        const outW = Math.round(baseW * renderScale);
        const outH = Math.round(baseH * renderScale);
      ...
      img.onerror = () => reject(new Error("Failed to load SVG XML"));
      img.src = item.previewUrl;
    });
  };

  // Batch convert all SVG to JPG
  const handleConvertAll = async () => {
    if (images.length === 0) {
      toast.error("Please upload at least one SVG graphic.");
      return;
    }

    setProcessing(true);
    setStatusMessage("Rasterizing vector SVGs into high-resolution JPGs... 0%");
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        const item = images[i];
        setStatusMessage(`Rasterizing graphic ${i + 1} of ${images.length} at ${scale}x scale...`);
        const convertedBlob = await convertSingleSvgToJpg(item, scale, quality, bgFill);
      ...
      setImages(updated);
      setIsConverted(true);
      toast.success(`All SVGs rasterized to ${scale}x JPG successfully!`);
    } catch (err) {
      console.error(err);
      toast.error("Failed to convert one or more SVG files.");
    } finally {
      setProcessing(false);
    }
  };
  ...
  const downloadSingleJpg = (item: ConvertItem) => {
    ...
    a.download = `${baseName}_${scale}x.jpg`;
  ...
  const downloadAllAsZip = async () => {
    ...
    setProcessing(true);
    setStatusMessage("Bundling rasterized JPG files into ZIP archive...");
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        if (item.convertedBlob) {
          const baseName = item.file.name.replace(/\.[^/.]+$/, "");
          zip.file(`${baseName}_${scale}x.jpg`, item.convertedBlob);
        }
      }
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `converted_svg_jpgs_${Date.now()}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
    ...
    <div className="mx-auto max-w-6xl px-3 sm:px-4 py-6 sm:py-10">
      ...
      {images.length === 0 && (
        <div className="mb-8 text-center">
          ...
          >
            ...
            Vector SVG to High-Resolution JPG Rasterizer
          </span>
        ...
        <label className="upload-dropzone upload-dropzone-image w-full relative group cursor-pointer">
          ...
          <span className="text-center">
            <p className="text-base font-semibold text-zinc-800 dark:text-zinc-100">
              Click or drag SVG vector graphics here
            </p>
            <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
              Render vector illustrations into sharp 1x, 2x, or 4x high-resolution JPG images.
            </p>
      ...
      {images.length > 0 && (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-zinc-900 dark:text-white">
                    {images.length} SVG Graphic{images.length === 1 ? "" : "s"} Loaded
                  </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {images.map((item) => (
                ...
                >
                  ...
                  {/* Thumbnail */}
                  <div className="w-full h-36 rounded-lg bg-zinc-100 dark:bg-zinc-900 overflow-hidden flex items-center justify-center relative p-2">
                    <img
                      src={item.previewUrl}
                      alt={item.file.name}
          ...
          {/* Right Column: Scale, Quality & Settings */}
            ...
            <div className="flex flex-col gap-5 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
              <div>
                ...
                <p className="text-xs text-zinc-500">
                  Select render resolution multiplier and background color fill.
                </p>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="grid grid-cols-3 gap-2 text-xs">
                  {[
                    { val: 1, label: "1x Standard", desc: "Original size" },
                    { val: 2, label: "2x Retina", desc: "High Definition" },
                    { val: 4, label: "4x Ultra HD", desc: "Print & 4K" },
                  ].map((s) => (
                    <button
                      key={s.val}
                      type="button"
                    ...
                    >
                      ...
                      <span className="text-[10px] text-zinc-400 font-normal">{s.desc}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="rounded-xl border border-zinc-200 bg-zinc-50/80 p-3.5 dark:border-zinc-800 dark:bg-zinc-900/50 space-y-1 text-[11px] text-zinc-600 dark:text-zinc-400">
                ...
                <p>
                  SVGs are rasterized locally in your browser canvas. No files are uploaded to any server.
                </p>
              </div>
              ...
              >
                {processing ? (
                  <>
                    ...
                    <span>Rasterizing SVG to JPG...</span>
                  </>
                ) : (
                  <>
                    <FileType className="h-4 w-4" />
                    <span>Rasterize {images.length} SVG{images.length === 1 ? "" : "s"} to JPG</span>
                  </>
                )}
              </button>
            </div>
