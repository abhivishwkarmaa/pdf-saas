...
interface JpgToPngWorkspaceProps {
  tool: ToolDefinition;
}
...
export function JpgToPngWorkspace({ tool }: JpgToPngWorkspaceProps) {
  ...
  const [colorMode, setColorMode] = useState<"standard" | "enhanced">("standard");
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    toast.success(`Added ${newItems.length} JPG image(s)!`);
  };
  ...
  // Convert single JPG to PNG using browser canvas
  const convertSingleToPng = (item: ConvertItem): Promise<Blob> => {
    return new Promise((resolve, reject) => {
      ...
      img.onerror = () => reject(new Error("Failed to load source JPG"));
      img.src = item.previewUrl;
    });
  };

  // Batch convert all to PNG
  const handleConvertAll = async () => {
    ...
    setProcessing(true);
    setStatusMessage("Converting JPG photos to lossless PNG... 0%");
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        const item = images[i];
        setStatusMessage(`Converting image ${i + 1} of ${images.length} to PNG...`);
        const convertedBlob = await convertSingleToPng(item);
      ...
      setImages(updated);
      setIsConverted(true);
      toast.success("All JPGs converted to lossless PNG format!");
    } catch (err) {
      console.error(err);
  ...
  const downloadAllAsZip = async () => {
    ...
    setProcessing(true);
    setStatusMessage("Bundling converted PNG files into ZIP archive...");
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `converted_pngs_${Date.now()}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
        ...
        <label className="upload-dropzone upload-dropzone-image w-full relative group cursor-pointer">
          ...
          <span className="text-center">
            <p className="text-base font-semibold text-zinc-800 dark:text-zinc-100">
              Click or drag JPG/JPEG photos here to convert to PNG
            </p>
            <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
              Export lossless 24-bit PNG images without compression artifacts.
            </p>
            ...
            <div className="flex flex-col gap-5 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
              <div>
                ...
                <p className="text-xs text-zinc-500">
                  Converts lossy compressed JPEGs into uncompressed 24-bit PNG format.
                </p>
              </div>
              ...
              <div className="rounded-xl border border-zinc-200 bg-zinc-50/80 p-3.5 dark:border-zinc-800 dark:bg-zinc-900/50 space-y-2 text-xs text-zinc-600 dark:text-zinc-400">
                <div className="flex items-center gap-2">
                  ...
                  <span>Full 24-bit RGB True Color encoding</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="h-4 w-4 text-emerald-600 dark:text-emerald-400 shrink-0" />
                  <span>No generational loss upon repeated saves</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="h-4 w-4 text-emerald-600 dark:text-emerald-400 shrink-0" />
                  <span>100% compatible with Photoshop, Canva & Figma</span>
                </div>
              </div>
              ...
              >
                {processing ? (
                  ...
                  <>
                    ...
                    <span>Convert {images.length} JPG{images.length === 1 ? "" : "s"} to PNG</span>
                  </>
                )}
              </button>
            </div>
